...
public final class PackageUtils {
     ...
     * @return 符合条件的类集合
     */
    public static Set<Class<?>> listClazz(String packageName, boolean recursive, Predicate<Class<?>> predicate) {
        ...
        try {
            ...
            while (urlEnum.hasMoreElements()) {
                ...
                if ("FILE".equalsIgnoreCase(protocol)) {
                    // 从文件系统中加载类
                    tmpSet = listClazzFromDir(new File(currUrl.getFile()), packageName, recursive, predicate);
                    ...
                    // 从 JAR 文件中加载类
                    tmpSet = listClazzFromJar(new File(fileStr), packageName, recursive, predicate);
                }
     ...
     * @param predicate   类过滤器
     * @return 符合条件的类集合
     */
    private static Set<Class<?>> listClazzFromDir(final File dirFile, final String packageName, final boolean recursive, Predicate<Class<?>> predicate) {
        ...
        while (!fileQ.isEmpty()) {
            ...
            try {
                ...
                if (null != predicate && !predicate.test(clazzObj)) {
                    // 如果过滤器不为空,
                    // 且过滤器不接受当前类,
                    // 则直接跳过!
                    continue;
     ...
     * @return 符合条件的类集合
     */
    private static Set<Class<?>> listClazzFromJar(final File jarFilePath, final String packageName, final boolean recursive, Predicate<Class<?>> predicate) {
